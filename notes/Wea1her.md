...
-   **可执行的协议与智能合约（Enforceable Agreements & Smart Contracts）：** AI 需要能够缔结并执行协议，且不需要依赖中心化的中间人。通过以太坊的智能合约和可编程货币（Programmable money），人类能够为 AI 设定严格的“规则和护栏”。例如，你可以给 AI 分配一个仅装有 10 美元的智能合约，并从代码层面限制它只能向某一家特定的供应商付款买披萨，从而杜绝 AI 乱花钱或执行错误操作的风险。
    
-   **隐私保护与零知识证明（Privacy & ZKML）：** 遵循以太坊的“CROPS”原则（抗审查、开源免费、隐私和安全），隐私是不可或缺的基础设施。资料中特别提到了 **ZKML（零知识机器学习）** 以及扩展的私人支付基础设施，这些技术允许 AI 智能体在验证某些信息或计算结果时，无需泄露底层的敏感数据。此外，未来还会引入基于 TEE（可信执行环境）的验证注册表来增强安全性。
    
-   **自动化谈判机制（Automated Negotiation）：** 随着智能体经济的发展，以太坊的路线图还包括构建智能体之间、以及智能体与智能合约之间的“自动化谈判”基础设施，让机器之间能够直接在链上商定条款并达成合作。
    
-   **去中心化协调与治理机制（Coordination & Governance）：** AI 被视为一种新的经济参与者，需要在一个中立的公共层面上与其他人或机器进行大规模的协调（Coordinate）。这就需要相应的治理（Governance）基础设施，确保 AI 智能体的运行规则是公开的，并且能够随时被人类审查和治理。
    
-   **AI 框架与企业级工具的集成（Integration with AI Frameworks）：** 为了让这些链上机制真正落地应用，以太坊还需要进一步加强与现有的 AI 框架以及各种企业级工具的深度整合，从而将 AI 的链下能力与以太坊的链上确权、支付能力无缝结合。
