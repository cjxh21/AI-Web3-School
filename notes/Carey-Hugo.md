...
| 任务 1 | Cobo Agentic Wallet SDK 跑通 | ✅ |
...
> 普通 WaaS 走 portal.cobo.com（昨天 6/2 走错了）；
> Agentic Wallet 走 caw CLI + App 配对，不经过 Portal。

**关键知识点：**

1. **Cobo 产品线分布**
   - Cobo WaaS 2.0：传统钱包（资产/MPC/合约钱包），developers.cobo.com
   - Cobo Agentic Wallet：Agent 专用，cobo.com/products/agentic-wallet
...
3. **Pact 协议**
   - Agent 提交"任务" → Pact 定义 Intent / Execution Plan / Policies
   - Owner 在 App 审批 → Pact active
...
**与创客星球(CGHub)的关联：**
> CGHub 收益分配钱包的设计，可以直接套用 Agentic Wallet 的
> "Agent 持有钱包 + Pact 任务级授权"模式。
> 这比自建合约钱包 + 多签 + 政策引擎省 80% 工作量。

---

## 🛠 今日实操记录

### 1. caw CLI onboard（1 分 4 秒跑通）
...
```
Day 15/30· Week 3 黑客松正式启动

今日完成：
1. WCB Week 3 任务提交（4项，80分）：
   - ✅ Hackathon Direction Card（20pt）
   - ✅ Repo Skeleton + README.md（20pt）
   - ✅ Week 4 Sprint Plan（20pt）
   - ✅ 组队状态确认（20pt）

2. CGHub 黑客松正式开工：
   - 方向锁定：Cobo · Agentic Economy 赛道
   - Demo 核心：贡献记录 → x402证明 → Cobo Agentic Wallet 自动分配
   - Repo 结构建立：contracts/ + frontend/ + agent/ + sdk-integration/

3. WCB API 提交流程跑通：
   - 确认 API Key 有效：w3cb_sk_j55opvstQ6TtB0GwSzDoIQ-hdEpiMQTi
   - 修复参数名：idOrSlug（非programId）、需trackId

今日洞见：
- 黑客松是演示，不是开发完整产品——核心是让人看到最小闭环
- Cobo Agentic Wallet = AI Agent 自己持链上身份，CGHub 贡献分配场景完美吻合

明日计划：
- Cobo Agentic Wallet SDK 对接
- Proposal Memo + Scope Review + Risk Memo（60pt）
- 参加 Openday 直播（20:00-21:00）
```
...
1\. W3-01 Chain-aware Context - 理解了 AI Agent 读链的完整链路：viem → Alchemy RPC → 链上数据。查了 ETH 余额（0 ETH）、Block Number（10958623）、Gas Price。

2\. W3-02 Web3 Tool Use - 理解了 ABI 是合约的"接口说明书"，用 viem.readContract 成功调用真实 WETH9 合约（0xfff9976782d46cc05630d1f6ebab18b2324d6b14），查到 Vitalik 地址有 0.054 WETH + 58 ETH。

3\. 核心收获 - 读链（view 函数）不签名不花钱；写链需要签名和 Gas；ERC-20 代币需要用合约地址 + ABI 单独查询。

4\. CGHub 应用 - 为黑客松 MVP 的"AI Agent 自动记录贡献 → 读链验证 → 链上存证"闭环打下基础。

GitHub 链接： [https://github.com/Carey-Hugo/ai-web3-school-cghub/blob/master/daily/2026-05-31.md](https://github.com/Carey-Hugo/ai-web3-school-cghub/blob/master/daily/2026-05-31.md)
...
**核心产出：**  
\- 愿景标准表述：**和合共生、天下大同**（统一6个文件）  
\- 使命表述升级：人类文明的**未来新形态**探路  
\- 入门手册重编：19章→12章目录，合并使命/愿景/理念/价值观/行动宣言/Slogan为一个章节  
\- 精美HTML排版完成：深色宇宙主题，7张配图（base64内嵌），微信可直接打开  
\- 创客星球初创期分工协作机制：OPC改名为正式机制  
...
**明日计划：**  
\- 推进黑客松团队组建（Solidity开发+前端+Agent各1人）  
\- 继续W2活动任务（每日共学）
...
```
今日核心完成：
1. W2-07 Governance 治理协作流程草图（规则制定/争议裁决/角色权限）
2. W2-08 方向深挖包与项目初步Proposal（整合W2-01~07产出）

关键知识点：
- 规则制定：提案门槛100星钻，讨论3天+投票3天，67%通过，33%法定人数
- 争议裁决：3名仲裁者，任期3个月，7天内裁决
- 投票权重：按星钻持有量（贡献越多→话语权越大）
- CGHub闭环：贡献记录→Agent追踪→x402证明→智能合约分配→Governance裁决

明日计划：继续刷W2活动任务（回放视频+实时参加）
```
...
今日学习主题：W2 启动 · Payment/Commerce + Agent 四大能力 + 区块链浏览器实战
...
线上活动打卡（5.28 Co-learning + [Z.AI](http://Z.AI) 1st）
...
CGHub-WCB-Submissions/W2-01-AI×[Web3问题地图.md](http://Web3问题地图.md)

W2-01 提交文档

CGHub-WCB-Submissions/[W2-04-Agent-Profile能力声明.md](http://W2-04-Agent-Profile能力声明.md)

W2-04 提交文档

CGHub-WCB-Submissions/[W2-05-Agent权限策略.md](http://W2-05-Agent权限策略.md)

W2-05 提交文档

CGHub-WCB-Submissions/[W2-06-Agent-Workflow-威胁模型.md](http://W2-06-Agent-Workflow-威胁模型.md)
