...
今天主要把 Hackathon 项目方向从“想法”往“规格”推进了一步。前面几天一直在交易解释、Agent 权限和钱包安全之间来回比较，今天先把主方向定为 Transaction Risk Explainer。这个方向的核心问题比较明确：很多 Web3 新手在签名前看不懂交易或授权，只能依赖钱包弹窗里的有限信息；如果 AI 能在签名前把交易目的、资产影响、授权对象和潜在风险解释清楚，就能降低误签和误授权的概率。

今天也把项目边界收窄了一些。第一版不做自动签名，不发交易，不接管钱包，也不碰私钥和助记词，只做信息读取、交易解析和风险解释。输入可以先从交易哈希、合约地址或待签名 payload 开始；输出则是普通用户能看懂的摘要，包括这笔操作可能在做什么、是否涉及 transfer / approve / contract write、可能影响哪些资产或权限、风险点是什么，以及用户应该去哪里验证。这样可以把 AI 的解释能力和 Web3 的可验证记录结合起来，同时保留 human-in-the-loop。

我今天补了 [project-spec-v0.md](http://project-spec-v0.md) 的第一版，把用户、问题、AI 角色、Web3 角色、MVP、风险边界和 demo 路径先写出来。下一步不是马上做完整产品，而是先完成一个最小实验：选一笔真实交易，读取基础信息，生成自然语言解释和风险提示，再用区块浏览器验证解释是否可靠。如果这条链路能跑通，再考虑支持待签名 payload、合约 ABI 解析或 Agent 权限检查。
<!-- DAILY_CHECKIN_2026-06-01_END -->

# 2026-05-30
<!-- DAILY_CHECKIN_2026-05-30_START -->

今天主要在整理 Hackathon 项目方向。前面几天学了长期记忆、Agentic Wallet、隐私、钱包权限和交易验证之后，我发现自己最感兴趣的问题不是“让 Agent 自动上链”，而是“在用户真正签名之前，Agent 能不能把风险解释清楚”。这个问题足够具体，也比较贴近 Web3 新手真实会遇到的场景：看不懂交易、看不懂授权、看不懂合约调用，只能凭感觉点确认。

所以目前我更倾向于先做 Transaction Risk Explainer。它的第一版不需要直接接管钱包，也不需要替用户签名，而是先做一个“解释层”：输入一笔交易、一个合约地址，或者一段待签名 payload，输出这次操作可能在做什么、涉及哪些资产或权限、是否包含 approve / transfer / contract write、风险点在哪里，以及用户下一步应该检查什么。这样既能用到 AI 的解释能力，也能保留 Web3 里最重要的 human-in-the-loop。

今天也把 MVP 边界想得更清楚了一点：先不做资产操作，不做自动交易，不碰私钥和助记词，只做链上信息读取、交易结构解析和风险解释。最小实验可以从一笔真实交易哈希开始，读取交易基本信息，结合区块浏览器或 RPC 返回的数据，生成一段普通用户能看懂的解释。判断这个实验是否成立，不看功能多不多，而看它能不能让用户回答三个问题：这笔交易想做什么？它会不会影响资产或授权？我可以去哪里验证它说的对不对？
