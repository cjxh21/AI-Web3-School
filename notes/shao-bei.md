...
课程共计四周，循序渐进学习 AI 与 Web3 基础知识、二者融合应用、实操进阶与黑客松筹备，最后开展项目冲刺与成果展示。梳理出自身 AI 知识薄弱项，将按照大语言模型、提示工程、上下文处理、检索增强生成、智能体、开发框架、多客户端协议、效果评估的顺序补齐短板。专属学习助手已部署完毕，明日正式投入日常学习辅助工作。

现存疑问

混用两款 Claude 工具后，发现沙箱环境存在共享内存调度、外部接口调用相关问题，考虑更换灵活性更强的 Hermes、OpenClaw 类智能体开展后续学习。
...
区块提议者：是轮值当 “班长” 的质检员，除了基础奖励，还能从区块产出中额外分成。

所以，没当上提议者的验证者绝不是白忙活：他们靠持续、正确地验证区块，获得稳定、主要的质押收益，这才是参与质押最核心、最可靠的收入来源。
...
分层存储记忆，留存用户习惯与过往对话，跨会话可调用。

支持自动沉淀工作流程为可复用技能，不断迭代优化。

架构简洁易部署，可自定义人设规则，适配各类办公开发场景。

区别普通大模型，具备自我进化能力，使用越久适配性越强。
