...
-   **传统证明系统介绍**：
    ...
    -   **证明的定义与架构**：证明是为命题真实性提供的足够证据或论证，可通过发行者（issuer）、持有者（HOLDER）、验证者（verifier）三个角色架构理解。如超市购物，超商是发行者，顾客是持有者，退货时店员是验证者，收据是证明。
        
    -   **系统脆弱性表现**：发行者可能误报消费金额，持有者可能篡改收据，验证者可能与他人串通通过假收据报销，在学历证书、财务证明、链上数据导入、DAO 投票、家庭联络本签名等场景都存在类似问题。
        
    -   **系统存在的挑战**：三方都有作恶可能，证明的真实性和可验证性存疑；验证时易过多揭露资料，存在信息安全风险。
        
-   **零知识证明概念引入**：
    
    -   **直观例子说明**：以 “威利在哪” 绘本为例，通过在大纸上开洞展示威利位置，验证者能确认找到威利但不知其在绘本中的具体位置，形象解释零知识证明。
        
    -   **数学角度阐述**：引入互动（interactive）和几率（probabilistic）元素，使证明有更多变化。如证明者主张年满 18 岁，验证者通过提问细节并随机抽问，若证明者不符合事实，在多次互动中易答不上问题。
        
    -   **特性介绍**：IP 证明有完整性（completeness）和可靠性（soundness），即主张正确验证者几乎永远接受，主张错误验证者几乎 100% 拒绝。零知识证明在此基础上，验证者看证明前后获得的讯息一样多，如验证年满 18 岁，零知识证明下验证者只知是否年满 18 岁。
        
-   **零知识证明相关术语**：
    
    -   **SNARK**：形容证明足够短且可快速验证，如用毕业证书证明毕业比拿出所有教科书和考卷证明更短。它是 IP 的分支，有完整性和可靠性，但无零知识特性。
        
    -   **ZK SNARK**：在 SNARK 基础上加上零知识证明元素，验证后验证者只知必要信息。其研究方向包括证明大小要小、产生和验证时间要快、应对量子电脑潜在威胁，迁移到后量子密码学相关安全密码系统。
        
-   **零知识证明应用案例**：
    
    -   **机器学习（ML）**：用零知识证明电路表达神经网络，可印证输入输出结果。在不同象限情况有不同应用，如第二象限模型参数隐私时，可证明模型厉害又不泄露参数；第四象限资料隐私时，可验证模型是否按指定使用；第三象限可验证 AI 模型输出是否正确。还有 easykl 框架和 biometrics 客户端 ZKML 应用。
        
    -   **影像领域**：相机塞入私钥为照片生成数字签章并加上零知识证明，照片被修改时证明验证不通过，可打击网络虚假影像资讯。
        
    -   **身份验证**：用 ZK ID 技术可最小化资料揭露，如证明满 18 岁时只让验证者知道是否满 18 岁。sdjot 形式存在问题，open AC 用零知识证明的匿名凭证形式可解决身份关联问题。
        
    -   **区块链**：在区块链上可用于隐私和扩容，如 NFT 热潮时以太坊主网拥塞，将交易移到 layer two，压缩成 proof 回到主网验证，可分流交易并确认合法性。
        
-   **零知识证明的争议与思考**：
    
    -   **TOKEN mixer 争议**：Toric cash 这类 TOKEN mixer 使用成本高且有争议，可能被用于洗钱等非法活动，隐私工具是双刃剑。
        
    -   **责任归属问题**：技术去中心化时出了事责任分散，安全密码系统可能阻碍执法机关调查，责任归属不明确。
        
    -   **隐私重要性**：以二战荷兰犹太人登记为例，说明隐私应是预设模式，应降低使用隐私工具成本，人人都有权选择使用隐私工具保护安全。
...
③ Agent 调 fetch\_data 工具

④ fetch\_data → GET /data → x402 Server

⑤ x402 Server → 402 + 付款要求

⑥ fetch\_data → CAW 发 USDC（Pact 范围内）→ Sepolia

⑦ Sepolia → 返回 tx hash

⑧ fetch\_data → 带 Payment-Proof 重发请求

⑨ x402 Server → 200 + 数据 + 签名
...
用户输入: 查 Uniswap 7天交易量

↓

Agent: 需要购买 Dune 数据，5 USDC，在预算内，自动支付...

↓

CAW: 支付完成，tx hash: 0xabc...
...
**1\. CAW SDK 排太晚（6/6–6/8 才碰）** 原计划 Go server → CAW → Agent loop，但 CAW 是唯一完全没碰过的黑盒，万一文档缺东西、测试网要审核，三天没缓冲就崩了。调整：现在/明天就做 CAW spike——只发一笔 Sepolia USDC 转账，拿到 tx hash 就算过。通了再排三天不慌，不通直接降级 ethers.js。

**2\. Agent loop 不用教 Agent 理解 402** 原计划 Agent 自己处理「请求→402→付款→重发」循环，这个写起来比看起来复杂（retry、timeout、tx 确认等待）。更省的做法：把 x402 交互封在 Agent 的工具函数 `fetch_data` 里，Agent 只看到数据回来，不知道花了钱。Agent loop 变成标准 tool-calling 模式，开发量小一半。

**3\. 端到端联调只留了一天（6/11），不够** 三个组件（Go server + CAW + Agent loop）第一次碰一起，八成端口不对、header 拼错、tx 时间超时。调整：Go server 写完当天用 curl 模拟全流程联调一次，后面接入 CAW 只替换「手动付款→SDK 付款」，风险拆成两块。
...
**Policy/Guard 能防住的**：所有最终要动链上资产的操作——越权转账、调用白名单外合约、交易重放。只要攻击路径最终要走链上执行，确定性规则就能拦。

**Policy/Guard 防不住的**：链上执行之前发生的事——AI 判断被影响（prompt injection、幻觉）、数据来源被伪造、规则边界内的合规攻击、协议层的 MEV。这些在 Guard 介入之前就已经「出问题了」，或者根本不经过 Guard。

一句话：**Guard 是链上执行的门卫，门卫之前的事它管不到。**
