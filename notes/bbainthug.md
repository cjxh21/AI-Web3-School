...
今天完成了 Week 3 的第一个小 artifact：AI x Web3 Agent Evaluation Rubric v0.1。

昨天的计划是先写一份最小 evaluation rubric，今天把它落成了 docs/[evaluation-rubric.md](http://evaluation-rubric.md)。

## **What I Built**

-   Added docs/[evaluation-rubric.md](http://evaluation-rubric.md)
    

## **What I Learned**

AI x Web3 Agent 的评估不能只看回答是否流畅，而要看它是否可靠、可验证、可追责。

我把评估维度先拆成 5 个：

-   Fact Accuracy：是否基于真实来源，而不是模型猜测；
    
-   Citation and Evidence：是否有 tx hash、block number、contract address、docs link 等证据；
    
-   Risk Awareness：是否识别签名、授权、permit、转账、合约写入等风险；
    
-   Permission Boundary：是否区分 read-only 和 write-chain，是否保留人工确认；
    
-   Reproducibility：是否记录输入、工具调用、输出、错误、时间、chainId、tx hash 和模型依据。
    

## **Key Takeaway**

> _一个 AI x Web3 answer 不是“说得像专家”就够了，它必须能被验证、能被追踪、能说明风险边界。_
...
-   新建 docs/[safety-checklist.md](http://safety-checklist.md)；
    
-   先写 Web3 tool use 前的 10 个检查项。
    

## **Public Post Draft**

Day 15 / AI x Web3 School

今天完成了 Week 3 的第一个小 artifact：AI x Web3 Agent Evaluation Rubric v0.1。

我现在越来越觉得，评估 AI x Web3 Agent 不能只看回答是否流畅，也不能只看它有没有调用工具。更重要的是：它的回答能不能被验证、能不能追踪、有没有守住风险边界。

我先把 rubric 拆成 5 个维度：

1.  Fact Accuracy：链上事实不能靠模型猜。
    
2.  Citation：重要结论要有 tx hash、block、contract、docs 或 API 来源。
    
3.  Risk Awareness：要识别签名、授权、permit、转账和合约写入风险。
    
4.  Permission Boundary：read-only 和 write-chain 必须分开，高风险动作必须人工确认。
    
5.  Reproducibility：输入、工具调用、输出、错误、时间和 chainId 都要能回放。
    

一句话总结：AI x Web3 answer 不是“说得像专家”就够了，它必须能被验证、能被追踪、能说明风险边界。
<!-- DAILY_CHECKIN_2026-06-03_END -->

# 2026-06-02
<!-- DAILY_CHECKIN_2026-06-02_START -->

# Daily Check-in - 2026-06-02
...
今天主要做 Week 3 方向重启和打卡整理，还没有新增 demo 或链上实验。

Week 3 的主题是 **Evaluation, Permission, and Safety**。相比前两周继续堆概念，这周应该把重点放到：怎么判断 AI x Web3 Agent 是否可靠、是否越权、是否能解释风险、是否留下可验证证据。

## Current Focus

本周要围绕三个方向推进：

-   Evaluation rubric：怎么评估一个 chain-aware answer 是否靠谱；
    
-   Safety checklist：Web3 工具调用前需要检查哪些权限和风险；
    
-   Failure cases：整理错误链、过期数据、缺 citation、unsafe approval、unsupported conclusion 等失败案例。
    

## What I Learned / Recalled

-   AI x Web3 Agent 不能只看“能不能回答”，还要看回答是否有来源、是否区分事实和解释。
    
-   涉及钱包、授权、转账、合约写入时，必须保留人工确认和权限边界。
    
-   Web3 工具调用的日志要记录 chainId、address、token、spender、amount、tx hash、RPC response、error、timestamp 和模型依据。
    
-   评估模型时不能只看表达是否专业，要检查事实准确性、安全红线意识、严谨性和可验证性。
...
-   学习 repo 还没有 push 到 GitHub；
    
-   测试网交易还没完成；
    
-   最小合约读写还没完成；
    
-   Week 3 的 evaluation rubric 还没整理成独立文档；
    
-   Safety checklist 还没落成可复用模板。
...
-   新建一份 `evaluation-rubric.md`；
    
-   先写 5 个维度：事实准确性、citation、风险意识、权限边界、可复现性；
...
Day 14 / AI x Web3 School

今天主要做 Week 3 方向重启。

这一周主题是 Evaluation、Permission 和 Safety。前面我已经学了钱包、链上支付、LLM、Agent、RAG、Memory、API Support、Web3 交易排查和 Permit phishing evaluation。现在要从“理解概念”转到“判断系统是否可靠”。

我现在的理解是：AI x Web3 Agent 不能只看能不能回答，还要看它是否有来源、是否区分事实和解释、是否守住权限边界、是否要求人工确认、是否留下可追踪日志。

明天准备先做一个最小 evaluation rubric：事实准确性、citation、风险意识、权限边界、可复现性。先把评估标准写清楚，再继续做具体案例。
...
今天没有新增系统学习，也没有完成新的 demo / 测试网 / GitHub push。

这条记录主要用于周末复盘：过去两周已经积累了不少 AI x Web3 学习笔记，但真正可公开提交的 proof 还需要补齐。
...
-   LLM 底层机制、QKV、prefill / decode、temperature；
    
-   Tool Use、Agent、Memory、RAG、Workflow；
    
-   AI x Web3 API Support FAQ；
    
-   Web3 交易排查：tx hash、nonce、approve / permit、盲签；
    
-   Permit phishing LLM evaluation；
    
-   [LI.FI](http://LI.FI) Intents Builder Challenge；
...
现在最缺的不是概念笔记，而是外部可见 proof：

-   学习 repo 还没 push 到 GitHub；
    
-   测试网交易还没完成；
    
-   最小合约读写还没完成；
    
-   [LI.FI](http://LI.FI) Intents 内容还没有变成可发布 thread / article；
...
-   把 repo link 写进下一条 log；
    
-   再考虑测试网交易和 [LI.FI](http://LI.FI) 内容。
    

## **Public Post Draft**

Day 13 / AI x Web3 School

今天没有新增系统学习，主要做周末复盘。

过去这段时间我已经整理了不少笔记：钱包、链上支付、LLM、Agent、RAG、Memory、API Support、Web3 交易排查、Permit phishing evaluation、[LI.FI](http://LI.FI) Intents，以及 AI 产品为什么不是简单接模型。

但现在最大的缺口也很清楚：概念笔记够了，外部 proof 不够。学习 repo 还没 push 到 GitHub，测试网交易和最小合约读写也还没补。

下一步不再继续扩新主题，先做一个最小动作：把本地学习 repo 推到 GitHub，拿到可提交链接。先让已有内容变成公开 proof，再继续补 Web3 实践。
