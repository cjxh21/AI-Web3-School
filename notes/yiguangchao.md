...
-   新增 docs/[README.md](http://README.md)，作为文档索引，整理架构、合约、部署、奖励池运维、图示和面试材料入口。
    
-   更新 [README.md](http://README.md)，把 docs/[README.md](http://README.md) 加进 Documentation Assets。
    
-   保留了你已有的 .gitignore 改动：docs/ 不再被忽略，所以现有 docs 文件现在可以一起提交到 GitHub
...
-   增加区块浏览器链接、合约地址、折叠 input data
    
-   前端校验交易哈希，后端校验支持网络
    
-   AI 调用失败时保留交易数据和风险结果
    
-   区分链上查询与 AI 生成 loading 状态
    
-   Markdown 生成抽离到独立模块，内容更完整
    
-   增加复制成功反馈
    
-   补充 [SECURITY.md](http://SECURITY.md)、[EXAMPLES.md](http://EXAMPLES.md)、README
...
-   [README.md](http://README.md)：补充常用命令，并说明没有 OPENAI\_API\_KEY 时也能演示链上查询、风险评估和规则版 Markdown
    
-   src/app/page.tsx：AI 调用失败时自动生成“本地规则版解释”，这样 Markdown 导出不会因为没配 AI Key 就消失
