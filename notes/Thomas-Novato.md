...
# AI x Web3 打卡笔记｜钱包与权限：执行型 Agent 的安全地基

## 01｜为什么要学这个

AI x Web3 产品最危险的地方，不只是模型会回答错误，而是模型的错误判断可能被执行成真实链上操作。

在普通 Web 产品里，误操作可能还能撤回、退款、重置账户；但在链上，一次错误授权、错误转账、错误 swap、错误合约调用，往往是不可逆的。

所以，钱包与权限层是所有执行型 Agent 的基础设施。它决定了：

-   Agent 只能读取数据，还是可以生成交易；
    
-   用户是否看得懂自己授权了什么；
    
-   权限是否受到资产、额度、合约、函数、时间限制；
    
-   Agent 出错后，用户能不能撤销、冻结或恢复；
...
> Agent 真正危险的地方，不是它会说错，而是它能把错误变成链上动作。

* * *

## 02｜第一性原理

Agent 不应该真正“拥有钱包”，它只应该拥有一组 **可限制、可审计、可撤销的能力**。

把私钥直接交给 Agent，是最脆弱、最危险的设计。更合理的方式是：

-   钱包、智能账户或权限模块持有资产和执行权；
...
> 允许 Agent 在 24 小时内，使用最多 100 USDC，在指定 DEX 上进行 swap，滑点不得超过 1%，不得调用 approve，不得转账给非白名单地址。
...
AI Wallet UX 的重点，不是把聊天框接到钱包，而是让用户清楚知道：
...
AI wallet 的体验目标不是“减少确认次数”，而是让每一次确认都变得有意义。

* * *

### 2\. Permission Policy

Permission Policy 是给系统执行层看的规则，而不是给用户看的愿望。

它必须能被程序检查，不能只靠模型自觉遵守。
...
AI 可以帮助生成 policy 草稿，但真正执行必须由钱包、智能账户、后端 guard 或合约模块校验。

* * *

### 3\. Session Key Flow

Session Key 是一种给特定任务临时使用的受限 key。
...
1.  用户用主钱包创建 session key；
    
2.  同时设置权限范围；
    
3.  Agent 使用 session key 发起操作；
...
Session key 的价值是减少重复签名，但它不能绕过风险控制。

如果额度太大、时间太长、函数太宽，本质上只是把主钱包风险换了一个名字。
...
它不关心模型为什么建议这笔交易，只关心这笔交易是否符合规则。

在 Agent 场景中，guard 可以检查：
...
> 用确定性规则拦住不该发生的动作，用人工确认处理规则覆盖不了的灰区。

* * *

### 5\. ERC-4337 Workflow

ERC-4337 让账户不再只是 EOA 私钥，而可以变成可编程的 smart account。

对 AI Agent 产品来说，它的价值在于：
...
ERC-4337 的重点不是“让 Agent 更方便转账”，而是让账户本身拥有可编程的权限和安全边界。

* * *

### 6\. Pre-transaction Simulation

签名前模拟交易，是 Agent 钱包必须具备的基础能力。

模型解释 calldata 只能作为辅助，真正的安全判断应该来自结构化解析和链上模拟。
...
模拟结果需要翻译成人能理解的语言，但关键字段不能靠模型自由发挥，而应该来自链上模拟和结构化解析。

* * *

### 7\. Recovery / Revocation

只要给 Agent 权限，就必须设计恢复和撤销。
...
不要等事故发生后才补撤销入口。撤销能力应该从第一天就存在。

* * *

## 04｜在 AI x Web3 产品中的位置

钱包与权限是执行型 Agent 的地基。

没有权限层，Agent 只能停留在问答和建议阶段；权限层做错，Agent 又会直接威胁用户资产安全。

一个靠谱的 AI x Web3 产品，应该按风险逐步开放能力：

### 第一阶段：只读

Agent 只能读取链上数据、余额、交易记录、授权状态。

### 第二阶段：生成交易草稿

Agent 可以生成交易建议，但必须由用户手动签名。

### 第三阶段：小额自动执行

允许小额、白名单、短期 session key 自动执行。
...
这一节的核心不是“Agent 怎么拥有钱包”，而是“Agent 如何在不直接控制用户资产的情况下安全执行任务”。

真正靠谱的 Agent 钱包设计，不应该依赖模型自律，而应该依赖：
...
最终目标是让 Agent 具备执行能力，但不让它拥有无限权力。

一句话记忆：

> Agent 不应该拥有资产，它只应该在被限制、被审计、可撤销的权限范围内执行任务。
