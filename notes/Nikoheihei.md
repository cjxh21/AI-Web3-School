...
去中心化AI基础设施：bittensor

ai agent+钱包：

ai驱动的链上分析工具：ai行为分析链（toB）

智能钱包和安全助手：blockaid。分析风险
...
没有适用于所有人的通用隐私解决方案。透明和隐私等级定义都很重要。默认情况下最好每个人都有某种隐私层。

隐私话题我认为有点敏感。但是主讲人明显比目前的web3主流（✖️金融）更贴近加密领域起源的初衷。发言也很大胆。

agent的隐私问题目前还是一片空白需要探索。

对AI建设者来说这当然是好事，但没有快速解决方案。我们永远会落后一步，因为它移动得太快。要跟上这样一个高度炒作、资金高度密集的行业非常困难。

提倡开源隐私的多样性也是防止power从一个口袋到另一个口袋。
...
toC的产品如果没有memory，那么用户留存率为45%。

toC端chatbot的记忆连续性很重要。（确实）
...
当agent在进行交易时，信任不止基于应用层，需要执行层。

agent钱包执行思路：agent请求-用户approve-agent得到签名的能力-完成

mpc，pact实现执行层的规范，recipe skill layer。

mpc：两方签名交易才能完成。

pact（边界）：1.intent 2.执行计划 3.风控（策略引擎强制每次执行进行校验）4.completion condition（ttl）。pack推送给用户，用户可修改。为了防止用户看不懂加了ai进行风险解读。（实际上还是挺有门槛的）

recipe（扩展）：已验证的执行路径。实际上避免了一定程度大模型胡言乱语。但是对于新协议需要用户手动维护。

hint：链上支付在跨国支付的应用较好，传统行业一般。未来web2和web3可能是半壁江山。

不知道跨国支付用户画像和龙虾使用者用户画像的重合度，感觉这个产品门槛挺高的。
