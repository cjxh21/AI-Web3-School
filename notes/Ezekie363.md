...
今天完成了hackathon的ppt制作和项目demo打磨
<!-- DAILY_CHECKIN_2026-06-08_END -->

# 2026-06-07
<!-- DAILY_CHECKIN_2026-06-07_START -->

今日学习：Day 21 学习完成（Hackathon 实战 · 功能迭代与体验优化）。AI 方向：为 ChainQuery Agent 新增三项查询能力——DeFi TVL（DeFiLlama API，12 个主流协议）、加密市场恐惧贪婪指数（[Alternative.me](http://Alternative.me) API）、代币价格 24h 涨跌幅（CoinGecko `include_24hr_change`）；代币支持从 8 种扩展至 37 种。Agent 架构方向：实现 **6 轮对话历史滑动窗口**——将历史 messages 跨轮传入 LLM，追问上次查询结果时不再重复发起链上付费请求，解决了「用户只是问问题却被扣费」的体验问题；同步优化 system prompt，明确指示 LLM 纯聊天场景不调工具。

收获 / 思考：今天最大的收获是对 **LLM tool calling 边界**的理解——Agent 的「自主决策」和「过度决策」之间的边界非常模糊。用户说「这个指数是整个市场的吗」时，LLM 判断需要查一次数据才能回答，触发付费，但用户只是追问。根本原因是每轮对话从零开始、没有上下文，LLM 看不到上一轮结果，只好重新查。加入历史窗口后 LLM 有了记忆，才能区分「需要新数据」和「已有数据可以直接回答」。这让我意识到：**Agent 的成本控制不只是预算上限，还需要对话上下文的设计**——让 Agent 知道自己已经知道什么，才不会浪费资源反复获取相同信息。

明日计划：准备 Hackathon 答辩演示流程 / 完善演讲稿问答练习
...
今日学习：Day 20 学习完成（Hackathon 实战）。AI 方向：x402 协议实战——完整跑通了「402 Payment Required → CawSigner EIP-712 MPC 签名 → Facilitator 链上结算 → tx hash 验证」的 Agent 自主支付全链路；理解了 Cobo Agentic Wallet 的双 Key 架构（onboarding key 提交 Pact，pact-scoped key 执行操作）以及 Pact 策略的参数级约束机制。Web3 方向：EIP-3009 TransferWithAuthorization——理解了链下授权与链上执行分离的设计，Facilitator 代执行、调用方无需持有 ETH 的工程优势，以及 nonce 机制防重放的安全保证。

收获 / 思考：今天最深的认知是 **Pact = 链上最小权限原则**——不只是「允许某类操作」，而是精确到参数级约束，超出范围在 MPC 节点层面就被拒绝。另一个收获是亲手踩坑验证了 onboarding key 和 pact-scoped key 的完全分离——让 Pact 撤销机制真正有效，而不是形式上的权限控制。
...
今日学习： Day 16 学习完成。Hackathon 实战：Cobo Agentic Wallet 配对——通过 `caw` CLI 完成钱包 onboard、手机 App 配对、测试网代币领取，建立了 CAW 钱包的完整使用流程；x402 协议实践：搭建 FastAPI + x402 中间件的付费数据服务器，理解了 facilitator/server/client 三者必须在同一条链上协作的核心约束，完成项目依赖安装与 SDK 连接验证。
...
明日计划：搭建 Mock 付费服务器（FastAPI + x402 中间件） + 理解 x402 PaymentRequired Header 的完整格式
...
收获 / 思考： 今天最重要的认知是「先定义业务流程，再定义资金流」——先写付款代码会导致状态缺口（缺少交付中/验收中/争议中等中间态）、退款路径缺失、验收条件和付款没有绑定，合约部署后这些问题很难修补。Evaluator 需要被评估这点也很关键：误判「完成」让资金错误释放，误判「失败」让合格交付被退款，两种错误都会让服务方和付款方失去对系统的信任，所以 Handbook 推荐组合 Evaluator 而不是把所有判断压给一个模型。

明日计划： AI × Web3 Bridge → 智能体身份（Agent Identity）
