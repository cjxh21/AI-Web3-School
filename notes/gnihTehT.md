...
![ec92a6f13d11c5c020dcde5dfd517a6d.png](https://raw.githubusercontent.com/IntensiveCoLearning/AI-Web3-School/main/assets/gnihTehT/images/2026-05-29-1780068163054-ec92a6f13d11c5c020dcde5dfd517a6d.png)

**解决什么问题** — 当用户想通过自然语言指令与智能合约交互时，AI 可以降低技术门槛，但"AI 说执行"到"链上实际发生"之间存在不可逆的权力交接点，本流程把这个边界明确可视化出来。

**哪些步骤由 AI / Agent 辅助** — 步骤 2–3（解析意图、生成合约地址与参数说明）和步骤 9（解析交易收据、提取 Gas 用量与事件日志）均由 AI 完成，用户无需手动查看 ABI 或原始日志。

**哪些步骤必须人工确认** — 两处标注 👁 的橙色节点：**第一关**是在钱包签名前审核合约内容（核对地址、金额、函数调用是否与意图一致）；**第二关**是交易上链后人工对照区块浏览器，确认结果符合预期，而非仅信任 AI 解读。

**如何验证最终结果** — 链上交易完成后，通过 Etherscan / Blockscout 等区块浏览器独立查询 tx hash，核验状态码（Success/Revert）、Gas 消耗、Transfer 事件，与 AI 返回的摘要交叉验证。

**主要风险点** — 🚨 最高风险是**私钥安全**：签名环节一旦被恶意脚本捕获，资产不可追回。⚠ 其次是**提示词注入**（攻击者通过合约元数据操控 AI 生成错误说明）和**测试/主网混淆**（测试通过后忘记切换网络直接在主网执行）；人工审核自身也存在误判风险，尤其在复杂合约参数下。
