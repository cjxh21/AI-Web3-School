...
今天做了 Hackathon 最重要的一步：把研究变成可执行的架构。

上午花了 3 小时深挖了三份 Agent 架构材料——Claude Managed Agent 的记忆系统源码、Claude Code SubAgent 的 FORK 机制、10 位顶级交易员的策略框架。下午用一张图把这三份研究全串进了 AgentPay 的技术方案里。

三个技术决策：

1\. 存储层用「文件优先」路线。CMA 的记忆架构证明了一件事——对 Agent 来说，结构化文件比向量数据库更实用。AgentPay 的用户消费记录同样不需要 Embedding，用 Markdown + 链上哈希就够了。这样用户打开一个 .md 文件就能看到所有消费历史。

2\. 后端用 SubAgent 集群而不是单体。CC SubAgent 的 FORK + Prompt Cache 机制启发：把 AgentPay 的后端拆成 Explore（搜索外部服务）、Verify（验证结果）、Plan（选最优方案）三个 SubAgent，不同任务用不同模型控成本，共享同一个 Prompt 前缀省推理开销。

3\. 预算控制 = 仓位管理。Top 10 交易员有一条共同铁律——胜率不重要，仓位管理才是护城河。映射到 AgentPay：用户怕的不是 Agent 花钱，怕的是不知道 Agent 花了多少、花在哪。MVP 先做 visibility（消费面板），再做 security（预算锁死）。

三份研究把 AgentPay 从「概念」推进到了「可实现的架构」。下一步：初始化项目仓库，开始写 Intent 编辑器。
<!-- DAILY_CHECKIN_2026-06-07_END -->

# 2026-06-05
<!-- DAILY_CHECKIN_2026-06-05_START -->

Hackathon 阶段：架构研究 → 技术方案

今天把三份 Agent 架构深度研究全部串进了 AgentPay 的技术方案：

1\. CMA 记忆系统（287行笔记）

\- 「文件优先，不用 Embedding」→ AgentPay 用户消费记录用结构化 JSON/Markdown 存储，不用向量数据库

\- 「Dreaming 后台扫描」→ 自动扫描历史 Intent 生成消费报告

2\. CC SubAgent 源码原理（322行笔记）

\- 「FORK + Prompt Cache」→ SubAgent 集群共享前缀，省推理成本

\- 「不同任务不同模型」→ Explore（搜 API）用便宜模型、Verify（验结果）用强模型

\- 「.md 定义 Agent」→ 用户的 Payment Intent 持久化复用

3\. Top 10 交易员策略（241行笔记）

\- 「仓位管理 = 预算控制」→ 交易员保命的逻辑直接搬进 AgentPay 的预算层

关键产出：三份研究把 AgentPay 从「概念」推到了「可实现的架构」—— Intent 编辑器（前端）→ SubAgent 集群（后端编排）→ 记忆&报告（存储层），三层分工清晰。

Week 3 进度：D1-D3 缺席 / D4 选题 / D5 产品诊断 / D6 架构研究串连 → 下一步初始化项目仓库。
...
三份研究把 AgentPay 从「概念」推到了「可实现的架构」。Week 3 核心认知：Hackathon 不是从零写代码，是把 Bridge 概念 + 产品方法论 + Agent 架构研究全部灌进一个项目。下一步初始化仓库，开始写 Intent
...
今天把两个方法论同时用到了 Hackathon 项目上：

1\. DemandCurve 六步诊断重新审视方案：原本想做"四层预算锁死"的安全工具，诊断后发现用户的第一需求不是 security 而是 visibility——用户怕的不是 Agent 花钱，是"不知道 Agent 花在哪"。MVP 调整为先做 Intent 编辑器（让用户能表达）+ 消费面板（让用户能看见）。

2\. Top 10 交易员的仓位管理映射：交易员靠仓位保命，AgentPay 靠预算控费——同一条逻辑。Crisian 单笔 ≤1%、BitLung 三分法、滚仓日限额，全能映射到预算控制层。

下一步：初始化项目仓库，搭建 Next.js + wagmi 前端，先做 Intent 编辑器原型。Hackathon 10 天倒计时。
...
进入 Hackathon 项目阶段。选题：AgentPay——帮用户管理 Agent 消费的工具。

问题：Agent 能花钱，用户控制不了。方案：把 Week 2 学的 Agentic Commerce 六个节点变成产品——自然语言 → Payment Intent → 四层预算锁死 → ERC-8183 托管 → 链上收据可查。

技术栈：Next.js + wagmi（前端）、Solidity ERC-8183 兼容 Escrow（合约）、x402 + Base/USDC（支付）。Hackathon MVP 目标：连接钱包 → 输入任务 → 自动生成 Intent → 查看消费记录。
...
Agentic Commerce 的完整链路：Payment Intent（结构化的「买什么」）→ Budget Control（四层防火墙）→ x402 付款 → ERC-8183 锁钱 → Proof of Task Completion（Evaluator 验收）→ Escrow 放款/退款 → On-chain Receipt 存证。每一步都对应具体协议和合约调用，不再是空中楼阁。

2\. Week 1 的 12 个 Bridge 概念全映射回来了

最大的认知收获：Week 1 学的那些抽象概念，在 Agentic Commerce 这个真实场景里全部找到了具体位置。Agent Wallet 就是 Payment Intent 里的 chain 字段，Verifiable AI 就是 Evaluator 合约里的 schema 检查，Settlement & Escrow 就是 ERC-8183 的状态机。概念在场景里活过来了。

下周：启动 Hackathon proposal，暂定「买家侧」方向——帮普通用户管理 Agent 的消费意图、预算和审计，让「Agent 替我花钱」这件事变得可控、可追溯。
...
Week 2 Day 5 | 赛道：Agentic Commerce | Week 2 收尾

Week 2 完成了 Agentic Commerce 的完整学习闭环：六个知识节点 → 两个核心协议（x402 + ERC-8183）→ Payment Intent Schema → 全流程时序图。

最大收获：Week 1 的 12 个 Bridge 概念在 Agentic Commerce 里全部找到了「家」。Agent Wallet 不再是概念——它就是 Payment Intent 里 chain: "base"；Verifiable AI 不再是论文——它就是 Evaluator 合约里 schema\_mismatch → reject()。概念在真实场景里活过来了。

下一步：启动 Hackathon proposal，暂定方向是买家侧（Intent 管理 + 消费审计面板），解决普通用户「让 Agent 帮我花钱但不担心被坑」的问题。
